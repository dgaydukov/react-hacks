...
/**
 * Generate json file with any number of rows for testing purpose
 */


const fs = require('fs');

const generateJsonFile = (rowNumber) => {
    const generateHash = (n) => {
        var text = "";
    ...
    for(let i = 1; i <= rowNumber; i++){
        list.push({
            ...
            productName: generateHash(10),
            productHash: generateName(),
        })
    };
    
    
    fs.writeFile('data.json', JSON.stringify(list), 'utf8', (err)=>err && console.log(err));
}


generateJsonFile(100000);
